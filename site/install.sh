...
BIN="qatest"
INSTALL_DIR="${QATEST_INSTALL_DIR:-$HOME/.local/bin}"
GITHUB_RAW="https://raw.githubusercontent.com/Simonethg/qatest/main/latest.json"
GITHUB_REL="https://github.com/Simonethg/qatest/releases/latest/download/latest.json"

main() {
  echo ""
  echo "  qatest installer"
  echo "  the agent runtime for QA"
  echo "  github.com/Simonethg/qatest"
  ...
  if [ "$OS" = "Linux" ] && [ "$(uname -o 2>/dev/null || true)" = "Android" ]; then
    err "Android/Termux is not supported by qatest release binaries. SSH to a supported host instead."
  fi
  ...
  if [ -n "${QATEST_LOCAL_BIN:-}" ] && [ -f "${QATEST_LOCAL_BIN}" ]; then
    log "installing local binary ${QATEST_LOCAL_BIN}"
    mkdir -p "$INSTALL_DIR"
    cp "${QATEST_LOCAL_BIN}" "${INSTALL_DIR}/${BIN}"
  ...
  MANIFEST=""
  if [ -n "${QATEST_MANIFEST_URL:-}" ]; then
    MANIFEST="$(curl -fsSL --retry 3 --connect-timeout 10 --max-time 20 "$QATEST_MANIFEST_URL")" \
      || err "can't reach ${QATEST_MANIFEST_URL}"
  else
    for url in \
  ...
  if [ -z "$MANIFEST" ]; then
    err "can't reach a latest.json. Set QATEST_MANIFEST_URL, or build from source: cargo install --git https://github.com/Simonethg/qatest"
  fi
  ...
  if [ -z "$URL" ] || [ "$URL" = "null" ]; then
    err "release manifest does not include a binary for ${TARGET}. Build from source: cargo install --git https://github.com/Simonethg/qatest"
  fi
  ...
  if [ "$ACTUAL_SHA256" != "$SHA256" ]; then
    err "downloaded qatest checksum did not match"
  fi
...
finish() {
  ...
  if command -v "$BIN" >/dev/null 2>&1 || [ -x "${INSTALL_DIR}/${BIN}" ]; then
    echo ""
    log "ready. run 'qatest' to get started."
  fi
  echo ""
}
...
need() {
  if ! command -v "$1" >/dev/null 2>&1; then
    err "requires '$1' — install it first, or download a binary from https://github.com/Simonethg/qatest/releases"
  fi
}
